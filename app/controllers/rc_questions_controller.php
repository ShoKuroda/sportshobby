<?php
class RcQuestionsController extends AppController{
	var $name = 'RcQuestions';
	var $uses = array('RcQuestion','RcAnswer','Category');
	var $paginate = array(
		'conditions' => NULL,
		'fields' => "RcQuestion.*",
		'limit' => 50,
		'order' => array('RcQuestion.created' => 'desc'),
	);
	...
	function admin_list() {
		//テーブル名を定義
		$TABLE_NAME = "RcQuestion";
		$this->set('TABLE_NAME', $TABLE_NAME);
		$POST_COLUMN = array("category","nick_name","question_title","question_detail","display");
		$IMG_CNT = 3;
	...
	function ajax_question() {
		...
		if($this->RequestHandler->isAjax()) {
			$field = array(
				"fields"=>array("question_title","question_detail"),
				"conditions"=>array("RcQuestion.id ="=>$_POST["id"]),
				);
			
			$question_data = $this->RcQuestion->find('first',$field);
			$this->set('question_data', json_encode($question_data["RcQuestion"]));
		}		
	}
